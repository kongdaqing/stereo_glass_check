...
GridCalibrationTargetAprilgrid::GridCalibrationTargetAprilgrid(size_t rows, size_t cols) :
    GridCalibrationTargetBase(rows, cols), _tagCodes(AprilTags::tagCodes36h11) {}
...
//        // and exit
        std::cerr << "\n[ERROR]: Found apriltag not belonging to calibration board. "
                     "Check the image for the tag and hide it.\n";
  ...
  for (unsigned int i = 0; i < detections.size(); i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (subpix_displacement_squarred <= _options.maxSubpixDisplacement2) {
        ...
        std::cerr << "Subpix refinement failed for point: " << pIdx[j] << " with displacement: "
